...
This repository provides a compact C-based LDPC simulation to compare three decoders under the same channel and matrix setup:
...
The program sweeps SNR from **4.5 dB to 7.0 dB** and writes BER/FER/average iterations to `results/performance.csv`, then automatically generates `results/performance.svg` (and optionally `results/performance.png`).
...
RMAS1 schedules check-node groups by group residual magnitude each iteration. RMAS2 sorts all checks by residual magnitude and refreshes neighboring variable beliefs immediately after each selected check update.
